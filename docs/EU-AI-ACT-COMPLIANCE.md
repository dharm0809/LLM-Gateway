...
| **Verification step** | Send several requests within the same session (using the `X-Session-Id` header) and call `GET /v1/lineage/sessions/{session_id}` to retrieve the timeline. Confirm each record has a `sequence_number` incrementing from zero and a `previous_record_hash` matching the prior record's `record_hash`. Call `GET /v1/lineage/verify/{session_id}` to run server-side chain verification and confirm all records pass. Inspect the `gateway_attempts` table via `GET /v1/lineage/attempts` and verify that denied or errored requests also appear. |
...
| **Gateway capability** | Cryptographic session chains provide tamper evidence for the entire audit trail. Each session uses a Merkle chain where the `record_hash` is computed as SHA3-512 over metadata fields (execution_id, policy_version, policy_result, previous_record_hash, sequence_number, timestamp). The genesis record uses a well-known zero hash, and each subsequent record chains from its predecessor. The chain verification API (`GET /v1/lineage/verify/{session_id}`) recomputes hashes server-side and reports any breaks. The lineage dashboard also provides client-side verification using the js-sha3 library. For access control, the gateway supports API key authentication via the `WALACOR_GATEWAY_API_KEYS` configuration. Caller identity can be resolved from request headers (`X-User-Id`, `X-User-Roles`, `X-Team-Id`), enabling integration with upstream identity providers and API gateways that perform JWT/SSO validation. The gateway records the resolved identity in every execution record for attribution. Tool input and output data are hashed with SHA3-512 at the gateway level, providing integrity verification for MCP and built-in tool interactions. |
...
| Variable | Purpose | Default | Compliance Relevance |
|---|---|---|---|
| `WALACOR_SESSION_CHAIN_ENABLED` | Enable SHA3-512 Merkle chain for session records | `true` | Art. 12, Art. 15 integrity |
