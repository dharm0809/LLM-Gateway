...
- Cache refresh helpers: `_refresh_attestation_cache()` clears + repopulates from DB; `_refresh_policy_cache()` calls `set_policies()` with fresh version; `_refresh_budget_tracker()` iterates DB budgets + removes deleted keys
